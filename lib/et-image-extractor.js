...
export async function extractETImagesAndData(etBuffer, options = {}) {
  ...
  // ── Step 6: Extract products (UUID matching first, positional fallback) ─
  // Priority:
  //   1. extractProductsFromRows() — uses DISPIMG UUID matching (Tier 0)
  //      which is the most accurate method. The OLE2 extractor provides a
  //      uuidMap that maps UUIDs from cellImages.xml to image objects. The
  //      DISPIMG formulas in spreadsheet cells reference these same UUIDs.
  //      This gives us exact 1:1 image-to-product matching.
  //   2. extractProductsPositionally() — fallback for products that couldn't
  //      get images via UUID matching. Uses yPos sort + 1:1 zipping.
  //      NOTE: yPos values are absolute EMU positions, not row numbers,
  //      so positional zipping alone is unreliable as a primary method.
  reportProgress(onProgress, 65, 'Extracting product data', 'UUID matching images to rows...');
  let products;
  if (sortedImagesByPosition && sortedImagesByPosition.length > 0) {
    // Primary: UUID matching via extractProductsFromRows
    products = extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, oleUuidMap, sortedImagesByPosition);
    // Fallback: for products without images, try positional zipping
    const unmappedCount = products.filter(p => !p.hasPreMappedImage).length;
    if (unmappedCount > 0 && sortedImagesByPosition.length > 0) {
      console.log(`[ET-IMAGE-EXTRACTOR] ${unmappedCount} products without UUID-matched images, trying positional zipping fallback`);
      const positionalProducts = extractProductsPositionally(xlsxBuffer, sortedImagesByPosition);
      // Merge: for products without images, use positional match if available
      if (positionalProducts.length > 0) {
        let positionalIdx = 0;
        for (let pi = 0; pi < products.length && positionalIdx < positionalProducts.length; pi++) {
          if (!products[pi].hasPreMappedImage) {
            const posProd = positionalProducts[positionalIdx];
            products[pi].hasPreMappedImage = true;
            products[pi].matchedViaUUID = false;
            products[pi].imageName = posProd.imageName || products[pi].imageName;
            products[pi].dataUrl = posProd.dataUrl || products[pi].dataUrl;
            products[pi].imageBuffer = posProd.imageBuffer || products[pi].imageBuffer;
            positionalIdx++;
          }
        }
        console.log(`[ET-IMAGE-EXTRACTOR] Positional fallback: filled ${positionalIdx} products with positional images`);
      }
    }
  } else {
    products = extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, oleUuidMap, sortedImagesByPosition);
