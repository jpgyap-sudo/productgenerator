...
function extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, uuidMap, sortedImagesByPosition) {
  ...
  // Pure sequential assignment: data rows get images in top-to-bottom
  // yPos order. Row 2 (first data row) gets image #1 (lowest yPos),
  // row 3 gets image #2, etc. This is the simplest and most intuitive
  // approach — images are arranged vertically in the spreadsheet, so
  // the topmost image belongs to the first product, the next image to
  // the second product, and so on.
  //
  // We ignore UUID/DISPIMG matching because it creates non-sequential
  // gaps (e.g., row 11 pulling image #2 out of sequence, causing all
  // subsequent rows to shift and get wrong images).
  if (sortedImagesByPosition && sortedImagesByPosition.length > 0) {
    console.log(`[ET-IMAGE-EXTRACTOR] Building rowImageMap: ${sortedImagesByPosition.length} images → sequential assignment to data rows (starting at row ${startRow + 1})`);
    ...
    for (let r = startRow; r < data.length && imgIdx < sortedImagesByPosition.length; r++) {
      ...
      console.log(`[ET-IMAGE-EXTRACTOR] Map: row ${spreadsheetRow} → "${img.name}" (seq #${imgIdx + 1}, y=${img.yPos})`);
      imgIdx++;
    }
    console.log(`[ET-IMAGE-EXTRACTOR] Sequential mapping: ${imgIdx} images assigned to ${imgIdx} data rows`);
  } else {
  ...
  for (let r = startRow; r < data.length; r++) {
    ...
    let rowImage = null;
    let matchedViaUUID = false; // Always false — UUID matching skipped for sequential assignment

    // Use the rowImageMap built earlier (sequential assignment).
    // We intentionally skip UUID matching because DISPIMG formulas are at
    // non-consecutive rows (11, 19, 27, ...) which breaks sequential order.
    rowImage = rowImageMap.get(spreadsheetRow);
    if (rowImage) {
      console.log(`[ET-IMAGE-EXTRACTOR] Row ${spreadsheetRow}: image "${rowImage.name}" (seq from rowImageMap)`);
    }
