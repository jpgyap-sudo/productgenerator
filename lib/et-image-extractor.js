...
//  OpenAI GPT-4o Vision visually verifies each product-image pair to catch
...
//
//  Design:
//    - Uses OpenAI GPT-4o Vision (gpt-4.1-mini default, gpt-4o for accuracy)
//    - Batch-by-batch processing: 2 products at a time (slower = more accurate)
//    - 3s deliberate pause between batches to avoid rate limits
//    - Retry with exponential backoff (15s → 45s → 90s)
//    - 45s timeout per product match
...
const ET_BATCH_SIZE = 2;            // Products per batch (slower = more reliable)
const ET_INTER_BATCH_DELAY_MS = 3000; // 3s pause between batches
const ET_MATCH_TIMEOUT_MS = 45000;  // 45s timeout per product match
const ET_MAX_RETRIES = 3;
const ET_RETRY_DELAYS = [15000, 45000, 90000]; // 15s, 45s, 90s
...
async function verifyProductImagePair(product, imageDataUrl) {
  ...
  if (!apiKey) {
    return { isMatch: true, confidence: 100, reason: 'AI verification skipped (no OpenAI key)' };
  }
  ...
  const prompt = `You are a product-image verification assistant for a furniture catalog. I will give you product information extracted from a spreadsheet row, and an image that was embedded in that same spreadsheet cell.

Your task: Determine if the image SHOWS the product described by the spreadsheet data.
...
CRITICAL RULES:
- The image should show the actual physical product (furniture item like a chair, table, sofa, etc.)
- The product type MUST match (e.g., if it says "dining chair", the image must show a dining chair)
- Product code may be visible on a label or tag in the image
- If the image shows a completely different product type, or is a logo/icon/graphic, it's NOT a match
- If the image is blurry or unclear but appears to show the right product type, lean toward matching
- If you're unsure, set confidence low rather than guessing

Return STRICT JSON ONLY with this exact structure:
{
  "match": true,
  "confidence": 94,
  "reason": "The image shows a modern dining chair with wooden frame matching the product description."
}
...
- Return ONLY valid JSON, no markdown, no code fences`;
  ...
  const executeVerify = async () => {
    ...
    console.log(`[ET-IMAGE-EXTRACTOR] AI verification: product "${product.name || product.productCode}" → ${isMatch ? 'MATCH' : 'MISMATCH'} (confidence: ${confidence}) — ${result.reason || ''}`);
  ...
  let lastError;
  for (let attempt = 0; attempt <= ET_MAX_RETRIES; attempt++) {
    try {
      ...
      if (isNonRetryable) {
        console.error(`[ET-IMAGE-EXTRACTOR] Non-retryable error for "${product.name}": ${err.message}`);
        return { isMatch: true, confidence: 100, reason: `AI verification unavailable: ${err.message}, trusting positional mapping` };
      }

      if (attempt >= ET_MAX_RETRIES) {
        console.warn(`[ET-IMAGE-EXTRACTOR] All ${ET_MAX_RETRIES + 1} attempts failed for "${product.name}": ${err.message}`);
        return { isMatch: true, confidence: 100, reason: `AI verification failed after retries: ${err.message}, trusting positional mapping` };
      }

      const delay = ET_RETRY_DELAYS[attempt] || ET_RETRY_DELAYS[ET_RETRY_DELAYS.length - 1];
      console.warn(`[ET-IMAGE-EXTRACTOR] Retry ${attempt + 1}/${ET_MAX_RETRIES} for "${product.name}" in ${(delay / 1000).toFixed(0)}s: ${err.message}`);
      await new Promise(resolve => setTimeout(resolve, delay));
    }
  }

  return { isMatch: true, confidence: 100, reason: 'AI verification failed, trusting positional mapping' };
}
