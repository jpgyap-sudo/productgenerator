...
function extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, uuidMap, sortedImagesByPosition) {
  ...
  // ── Build rowImageMap from OLE2 position-sorted images ────────────
  // Two-phase approach:
  //   Phase 1: UUID matching — for each DISPIMG formula at row N, map the
  //            corresponding image to row N. These are 100% accurate matches.
  //   Phase 2: Sequential assignment — for data rows without UUID-matched
  //            images, assign the next unassigned image in yPos order.
  //            This preserves top-to-bottom ordering without relying on
  //            yPos → row interpolation (which is unreliable because yPos
  //            values are absolute EMU positions, not row numbers).
  if (sortedImagesByPosition && sortedImagesByPosition.length > 0) {
    ...
    // Only mark images as "assigned" if they're mapped to actual data rows.
    // DISPIMG formulas may reference images at rows outside the data range
    // (e.g., row 451 when data only goes to row 58). Those images should
    // remain available for sequential assignment to data rows.
    const assignedImages = new Set(); // Track which images are already assigned
    const uuidMatchedRows = new Set(); // Track which rows have UUID-matched images
    const dataStartRow = startRow + 1; // 1-based
    const dataEndRow = data.length; // 1-based (last row index)
    if (dispimgMap.size > 0 && uuidMap && uuidMap.size > 0) {
      for (const [spreadsheetRow, uuid] of dispimgMap) {
        if (uuidMap.has(uuid)) {
          const img = uuidMap.get(uuid);
          rowImageMap.set(spreadsheetRow, img);
          uuidMatchedRows.add(spreadsheetRow);
          // Only mark as "assigned" if this row is within actual data range
          if (spreadsheetRow >= dataStartRow && spreadsheetRow <= dataEndRow) {
            assignedImages.add(img);
            console.log(`[ET-IMAGE-EXTRACTOR] UUID map: row ${spreadsheetRow} → "${img.name}" (${img.uuid}) [DATA ROW]`);
          } else {
            console.log(`[ET-IMAGE-EXTRACTOR] UUID map: row ${spreadsheetRow} → "${img.name}" (${img.uuid}) [outside data range ${dataStartRow}-${dataEndRow}, image remains available]`);
          }
        }
      }
    }
    console.log(`[ET-IMAGE-EXTRACTOR] UUID matching: ${uuidMatchedRows.size} formulas, ${assignedImages.size} images assigned to data rows`);

    // ── Phase 2: Sequential assignment for unmapped rows ─────────────
    // Collect unassigned images (sorted by yPos — they already are)
    const unassignedImages = sortedImagesByPosition.filter(img => !assignedImages.has(img));
    if (unassignedImages.length > 0) {
      console.log(`[ET-IMAGE-EXTRACTOR] Sequential assignment: ${unassignedImages.length} unassigned images for non-DISPIMG rows`);

      // Walk through data rows in order, skipping UUID-matched rows
      let imgIdx = 0;
      for (let r = startRow; r < data.length && imgIdx < unassignedImages.length; r++) {
        const row = data[r];
        if (!row) continue;
        ...
        // Skip rows that already have UUID-matched images
        if (uuidMatchedRows.has(spreadsheetRow)) continue;

        const img = unassignedImages[imgIdx];
        rowImageMap.set(spreadsheetRow, img);
        console.log(`[ET-IMAGE-EXTRACTOR] Sequential map: row ${spreadsheetRow} → "${img.name}" (y=${img.yPos})`);
        imgIdx++;
      }
      console.log(`[ET-IMAGE-EXTRACTOR] Sequential assignment: mapped ${imgIdx}/${unassignedImages.length} unassigned images to data rows`);
    }
  } else {
...
export async function extractETImagesAndData(etBuffer, options = {}) {
  ...
  // ── Step 6: Extract products (UUID matching + sequential assignment) ──
  // extractProductsFromRows() uses a two-phase approach:
  //   Phase 1: UUID matching — DISPIMG formulas in spreadsheet cells map
  //            directly to OLE2-extracted images via uuidMap.
  //   Phase 2: Sequential assignment — for data rows without UUID-matched
  //            images, assign the next unassigned image in yPos order.
  // This produces correct top-to-bottom image-to-product mappings without
  // relying on unreliable yPos interpolation.
  reportProgress(onProgress, 65, 'Extracting product data', 'UUID matching images to rows...');
  let products;
  if (sortedImagesByPosition && sortedImagesByPosition.length > 0) {
