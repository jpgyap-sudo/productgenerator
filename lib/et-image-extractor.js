...
export async function extractETImagesAndData(etBuffer, options = {}) {
  ...
  // ── Step 6.5: Smooth progress animation ──────────────────────────
  // After the real work is done, animate the progress bar smoothly
  // from current progress to 100% so the user sees a satisfying fill.
  // The animation is broken into sub-steps with delays, and checks
  // the pause flag between each sub-step.
  if (typeof onProgress === 'function') {
