...
function extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, uuidMap, sortedImagesByPosition) {
  ...
  if (sortedImagesByPosition && sortedImagesByPosition.length > 0) {
    ...
    for (let r = startRow; r < data.length && imgIdx < sortedImagesByPosition.length; r++) {
      ...
      console.log(`[ET-IMAGE-EXTRACTOR] Position-based map: row ${spreadsheetRow} → "${img.name}" (y=${img.yPos}, uuid=${img.uuid})`);
      imgIdx++;
    }
    console.log(`[ET-IMAGE-EXTRACTOR] Built rowImageMap with ${rowImageMap.size} entries from OLE2 position data (${imgIdx}/${sortedImagesByPosition.length} images mapped)`);
  } else {
...
export async function extractETImagesAndData(etBuffer, options = {}) {
  ...
  // Pass the OLE2 uuidMap and sortedImagesByPosition so extractProductsFromRows
  // can do UUID-based matching AND position-based row mapping.
  reportProgress(onProgress, 65, 'Extracting product data', 'Parsing spreadsheet rows...');
  const products = extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, oleUuidMap, sortedImagesByPosition);
