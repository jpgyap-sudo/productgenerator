...
import { matchRowsViaScreenshot } from './et-screenshot-matcher.js';
...
export async function extractETImagesAndData(etBuffer, options = {}) {
  ...
  // ── Step 5.5: Visual row alignment via Playwright screenshot ─────
  // Use Playwright to screenshot the .et rendered as PDF, then use AI
  // vision to visually determine which image belongs to which product row.
  // This overrides the positional mapping which can be unreliable after
  // LibreOffice conversion.
  if (products.length > 0 && images.length > 0) {
    try {
      reportProgress(onProgress, 82, 'Visual row alignment', 'Screenshotting spreadsheet...');
      console.log('[ET-IMAGE-EXTRACTOR] Attempting visual row alignment via Playwright screenshot...');

      const { rowImageMap: visualRowMap, method } = await matchRowsViaScreenshot(
        etBuffer, products, images, tempDir
      );

      if (method === 'visual_ai' && visualRowMap.size > 0) {
        console.log(`[ET-IMAGE-EXTRACTOR] Visual row alignment succeeded (${visualRowMap.size} mappings). Overriding positional mapping.`);

        // Apply visual mappings to products
        let appliedCount = 0;
        for (const product of products) {
          if (product.row !== undefined && visualRowMap.has(product.row)) {
            const visualImage = visualRowMap.get(product.row);
            product.imageName = visualImage.name;
            product.hasPreMappedImage = true;
            product.dataUrl = `data:${visualImage.mimeType || 'image/png'};base64,${visualImage.buffer ? visualImage.buffer.toString('base64') : ''}`;
            product._visualMatchConfidence = visualImage._aiConfidence || 0;
            product._visualMatchReason = visualImage._aiReason || '';
            appliedCount++;
          }
        }
        console.log(`[ET-IMAGE-EXTRACTOR] Applied ${appliedCount} visual row alignments to products`);
      } else {
        console.log(`[ET-IMAGE-EXTRACTOR] Visual row alignment not available (method: ${method}), using positional mapping`);
      }
    } catch (err) {
      console.warn(`[ET-IMAGE-EXTRACTOR] Visual row alignment failed (non-fatal): ${err.message}`);
      // Non-fatal — fall back to positional mapping
    }
  }
