 ...
 * @returns {Promise<{images: Array, rowImageMap: Map<number, object>}>}
 *   images: Array of { id, index, buffer, extension, mimeType, name }
 *   rowImageMap: Map<rowNumber, imageObject>
 */
async function extractImagesFromXLSX(xlsxBuffer) {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(xlsxBuffer);
  ...
  if (mediaList.length === 0) {
    return { images: [], rowImageMap: new Map() };
  }
  ...
  if (!worksheet) {
    ...
    return { images: mediaList, rowImageMap };
  }
  ...
  try {
    ...
    drawingImages.forEach((imgPlacement) => {
      ...
      // The anchor row tells us which spreadsheet row this image belongs to
      const spreadsheetRow = tl.row + 1; // Convert to 1-based
      ...
      if (!rowImageMap.has(spreadsheetRow)) {
        ...
        console.log(`[ET-IMAGE-EXTRACTOR] Mapped image #${mediaIndex} "${media.name}" to row ${spreadsheetRow}`);
      }
    });
  } catch (drawErr) {
 ...
 *
 * @param {Buffer} xlsxBuffer - Converted .xlsx file buffer
 * @param {Map<number, object>} rowImageMap - Map of rowNumber -> image object
 * @returns {Array<object>} Array of { row, productCode, name, description, brand, generatedCode, imageBuffer, imageName, dataUrl }
 */
function extractProductsFromRows(xlsxBuffer, rowImageMap) {
  ...
  const codeCol = findCol(['code', 'product code', 'item code', 'sku', 'no', 'part number']);
  const descCol = findCol(['description', 'desc', 'product description', 'item description', 'name', 'product name']);
  const brandCol = findCol(['brand', 'brand name', 'manufacturer', 'vendor']);
  const imageCol = findCol(['image', 'img', 'picture', 'photo', 'pic']);
  ...
  for (let r = startRow; r < data.length; r++) {
    ...
    // The spreadsheet row number (1-based, accounting for header)
    const spreadsheetRow = r + 1;

    // Check if this row has a mapped image
    const rowImage = rowImageMap.get(spreadsheetRow);
...
export async function extractETImagesAndData(etBuffer, options = {}) {
  ...
  if (!libreAvailable) {
    ...
    return {
      ...
      warning: 'LibreOffice is not installed. Install LibreOffice to extract embedded images from .et files. Falling back to text-only extraction.'
    };
  }
  ...
  try {
    ...
    return {
      ...
      totalImages: 0,
      hasEmbeddedImages: false,
      warning: `LibreOffice conversion failed: ${convErr.message}. Falling back to text-only extraction.`
    };
  }

  // ── Step 4: Extract images + cell anchors ────────────────────────
  const { images, rowImageMap } = await extractImagesFromXLSX(xlsxBuffer);
  ...
  if (images.length === 0) {
    ...
    return {
      ...
      totalImages: 0,
      hasEmbeddedImages: false,
      warning: 'No embedded images found in the .et file. Falling back to text-only extraction.'
    };
  }

  // ── Step 5: Extract products from rows ───────────────────────────
  const products = extractProductsFromRows(xlsxBuffer, rowImageMap);
