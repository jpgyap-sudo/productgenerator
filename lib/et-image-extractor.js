 ...
 */
function extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, uuidMap) {
  ...
  //   3. Map images to DISPIMG rows ONE-TO-ONE (no cycling).
  //      If there are more DISPIMG rows than images, the extra rows
  //      are left unmapped — they will fall through to Tier 2 (drawing
  //      anchor) or remain without a pre-mapped image. This prevents
  //      false matches where 57 products get cycled through only 4 images.
  ...
  // IMPORTANT: Map ONE-TO-ONE only. If there are more DISPIMG rows than
  // images, the extra rows are left unmapped (no cycling). This prevents
  // false matches where 57 products get cycled through only 4 images.
  // Products without a pre-mapped image will be flagged for review.
  const positionalImageMap = new Map();
  if (sortedImages.length > 0) {
    const mapCount = Math.min(sortedDispimgRows.length, sortedImages.length);
    for (let i = 0; i < mapCount; i++) {
      positionalImageMap.set(sortedDispimgRows[i], sortedImages[i]);
      console.log(`[ET-IMAGE-EXTRACTOR] Positional map: DISPIMG row ${sortedDispimgRows[i]} → image "${sortedImages[i].name}" (one-to-one)`);
    }
    if (sortedDispimgRows.length > sortedImages.length) {
      console.log(`[ET-IMAGE-EXTRACTOR] WARNING: ${sortedDispimgRows.length - sortedImages.length} DISPIMG rows have no matching image (not cycling — these products need review)`);
    }
  }
  ...
  for (let r = startRow; r < data.length; r++) {
    ...
    // Tier 0: UUID matching (from OLE2 ETCellImageData — most accurate)
    // Tier 1: Positional mapping (DISPIMG row index → image order, one-to-one)
    // Tier 2: Drawing anchor lookup
    // NOTE: No Tier 3 cycling fallback. If a product has no unique image,
    ...
    // Tier 1: Positional mapping (only if UUID matching failed)
    ...
    // This is a one-to-one mapping — no cycling. Extra products without
    // images will be flagged for review.
    if (!rowImage && positionalImageMap.has(spreadsheetRow)) {
...
export async function extractETImagesAndData(etBuffer, options = {}) {
  ...
  if (oleImages && oleImages.length > 0) {
    ...
    console.log(`[ET-IMAGE-EXTRACTOR] Using ${oleImages.length} OLE2-extracted images (bypassing LibreOffice image extraction)`);
    images = oleImages;
    rowImageMap = new Map(); // Not needed when we have UUID matching
    imageNameMap = new Map(); // Not needed when we have UUID matching
    reportProgress(onProgress, 60, 'Using OLE2 images', `${oleImages.length} images with UUID mappings`);
  } else {
  ...
  // ── Step 6: Extract products from rows ───────────────────────────
  // Pass the OLE2 uuidMap so extractProductsFromRows can do UUID-based matching
  reportProgress(onProgress, 65, 'Extracting product data', 'Parsing spreadsheet rows...');
  const products = extractProductsFromRows(xlsxBuffer, rowImageMap, imageNameMap, oleUuidMap);
