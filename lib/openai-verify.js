...
export async function verifyCandidates(product, candidates, pdfImageDataUrl = null) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (result.success) {
      console.log(`[OPENAI-VERIFY] Result: success=true, confidence=${result.data?.confidence}, status=${result.data?.status}, isMatch=${result.data?.isMatch}`);
    } else {
      console.log(`[OPENAI-VERIFY] Result: success=false, error=${result.error}`);
    }
...
export async function verifyAllProducts(products, candidateResults, pdfImages = [], options = {}) {
  ...
  // Build a lookup of images by index so we can attach dataUrl to candidates
  const allImages = options.allImages || [];
  ...
  for (let batchStart = 0; batchStart < products.length; batchStart += concurrency) {
    ...
    const batchResults = await Promise.allSettled(
      batchProducts.map(async (product, batchIdx) => {
        ...
        const candidates = (productCandidates?.candidates || []).map(c => {
          // Attach dataUrl from the original images array using imageIndex
          if (!c.dataUrl && allImages[c.imageIndex]) {
            return { ...c, dataUrl: allImages[c.imageIndex].dataUrl };
          }
          return c;
        });
