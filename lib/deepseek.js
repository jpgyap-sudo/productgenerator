...
/**
 * Pre-scan text for product code patterns using regex.
 * Catches codes like CH-790, HA-005, P01-01 that appear on their own line
 * in Chinese furniture catalogs. These are injected as hints into the DeepSeek prompt.
 *
 * @param {string} text - Raw PDF text
 * @returns {string[]} Array of detected product codes
 */
function scanForProductCodes(text) {
  const codes = new Set();
