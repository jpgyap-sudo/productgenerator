...
const MAX_TOKENS = 8192; // Increased from 4096 — batch responses with many products were getting truncated mid-JSON
  ...
  for (let i = 0; i < chunks.length; i += CONCURRENT_BATCHES) {
    ...
    for (let rIdx = 0; rIdx < results.length; rIdx++) {
      ...
      if (result.status === 'fulfilled' && result.value.length > 0) {
        ...
        const batchIdx = i + rIdx;
        const errorMsg = result.reason?.message || 'Unknown error';
        console.error(`[DEEPSEEK] Batch ${batchIdx + 1}/${chunks.length} failed: ${errorMsg}`);

        // Retry failed batches up to MAX_BATCH_RETRIES times
        let retrySuccess = false;
        for (let retry = 1; retry <= MAX_BATCH_RETRIES; retry++) {
          console.log(`[DEEPSEEK] Retrying batch ${batchIdx + 1}/${chunks.length} (attempt ${retry}/${MAX_BATCH_RETRIES})...`);
          await new Promise(resolve => setTimeout(resolve, 2000 * retry)); // Increasing delay
          try {
            const retryResult = await extractProductInfoBatch(
              batch[rIdx], batchIdx + 1, chunks.length
            );
            if (retryResult.length > 0) {
              allProducts.push(...retryResult);
              retrySuccess = true;
              console.log(`[DEEPSEEK] Batch ${batchIdx + 1}/${chunks.length} recovered on retry ${retry}`);
              break;
            }
          } catch (retryErr) {
            console.error(`[DEEPSEEK] Batch ${batchIdx + 1}/${chunks.length} retry ${retry} failed: ${retryErr.message}`);
          }
        }

        if (!retrySuccess) {
          console.error(`[DEEPSEEK] Batch ${batchIdx + 1}/${chunks.length} failed after ${MAX_BATCH_RETRIES} retries — products from this batch will be missing`);
        }
      }
    }
  }
