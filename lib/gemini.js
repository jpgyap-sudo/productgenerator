...
//
//  Model strategy (based on https://ai.google.dev/gemini-api/docs/pricing):
//  - Views 1-4 (standard product shots): gemini-2.0-flash-image-generation
//    ($0.15/image — fast, cost-effective batch/preview model)
//  - View 5 (interior scene): gemini-2.5-flash-preview-04-17
//    ($0.15/image — newer preview model with image generation support)
//
//  NOTE: gemini-2.5-pro does NOT support image generation (text-only).
//  Both image-capable models are priced identically at $0.15/image.
//
...
/**
 * Gemini model names for different view types.
 * Based on Gemini API pricing page (https://ai.google.dev/gemini-api/docs/pricing):
 *   - gemini-2.0-flash-image-generation: $0.15/image — views 1-4
 *   - gemini-2.5-flash-preview-04-17: $0.15/image — view 5 (premium interior scene)
 *
 * Note: gemini-2.5-pro is text-only and does NOT support image generation.
 */
const GEMINI_MODELS = {
  STANDARD: 'gemini-2.0-flash-image-generation', // Views 1-4 ($0.15/image)
  PREMIUM: 'gemini-2.5-flash-preview-04-17'      // View 5 ($0.15/image, preview)
};
...
/**
 * Generate a single view using Gemini API.
 *
 * For views 1-4, uses gemini-2.0-flash-image-generation ($0.15/image).
 * For view 5, uses gemini-2.5-flash-preview-04-17 ($0.15/image, preview).
 *
 * NOTE: gemini-2.5-pro does NOT support image generation (text-only).
 * Both image-capable models are priced identically at $0.15/image.
 *
...
export async function generateGeminiView(view, desc, imageUrl, resolution = '1K') {
  ...
  // Both models are $0.15/image. gemini-2.5-flash-preview-04-17 is the
  // newer preview model used for the complex interior scene (view 5).
