 ...
 *   - gemini-3.1-flash-image-preview (Nano Banana 2): views 1-3
 *     High-quality image generation with conversational editing
 *   - gemini-3-pro-image-preview: view 4 (premium interior scene)
 *     Premium model for complex scene generation
 */
const GEMINI_MODELS = {
  STANDARD: 'gemini-3.1-flash-image-preview', // Views 1-3 (Nano Banana 2)
  PREMIUM: 'gemini-3-pro-image-preview'       // View 4 (premium interior scene)
};
 ...
 *
 * For views 1-3, uses gemini-3.1-flash-image-preview (Nano Banana 2).
 ...
 */
export async function generateGeminiView(view, desc, imageUrl, resolution = '1K', brand = '') {
  const apiKey = GEMINI_API_KEY;
  if (!apiKey) throw new Error('GEMINI_API_KEY not configured');

  // Select Gemini 3 model based on view type
  // gemini-3.1-flash-image-preview (Nano Banana 2) for views 1-3,
  // gemini-3-pro-image-preview for view 4 (premium interior scene).
  const modelName = view.id === 4 ? GEMINI_MODELS.PREMIUM : GEMINI_MODELS.STANDARD;
