...
/**
 * Generate a single view using Gemini 2.5 Flash API.
 *
 * Uses gemini-2.5-flash-image for all views. Retries up to 3 times
 * on transient errors (503/429) with exponential backoff.
 *
...
export async function generateGeminiView(view, desc, imageUrl, resolution = '1K', brand = '', options = {}) {
  ...
  // CRITICAL: Prevent Gemini from adding text/annotations/watermarks to the generated image.
  // Gemini 2.5 Flash Image has a known behavior of returning the reference image with
  // text overlay when it misinterprets the instruction. This instruction must be explicit.
  promptText += `\n\nCRITICAL INSTRUCTION: You MUST generate a completely NEW photorealistic product render image. Do NOT simply annotate, label, or add text to the reference image. Do NOT return the reference image with text overlay. The output MUST be a freshly rendered product photograph showing the product from the requested camera angle on a clean background. Absolutely NO text, labels, watermarks, annotations, or captions of any kind should appear on the generated image.`;
  ...
  console.log(`[GEMINI] View ${view.id} (${view.label}) — ${GEMINI_MODEL}`);
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (fetchErr.name === 'AbortError') {
        throw new Error(`Gemini timeout after ${Math.round(requestTimeout / 1000)} seconds`);
      }
      throw fetchErr;
    }
    clearTimeout(timeoutId);
    ...
    if (res.status === 503 || res.status === 429) {
      ...
      if (attempt < MAX_RETRIES) {
        ...
        console.log(`[GEMINI] ${res.status} on attempt ${attempt} — retrying in ${delay / 1000}s`);
        await new Promise(r => setTimeout(r, delay));
        continue;
      }
      console.log(`[GEMINI] ${res.status} exhausted after ${MAX_RETRIES} attempts`);
      throw lastError;
    }
    ...
    console.log(`[GEMINI] View ${view.id} succeeded`);
...
async function uploadGeminiResult(buffer, mimeType, viewId) {
  ...
  const fileName = `gemini_renders/view${viewId}_${Date.now()}_${nanoid(8)}.${ext}`;
