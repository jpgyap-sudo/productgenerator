...
async function analyzeScreenshotWithAI(screenshotDataUrl, products, images, pageIndex) {
  ...
  const prompt = `You are analyzing a screenshot of a spreadsheet/catalog page from a furniture company's .et file (WPS Spreadsheet).

The spreadsheet has product rows. Each row contains product information.

PRODUCTS extracted from this spreadsheet (${products.length} total):
${productList}

IMAGES extracted from this spreadsheet (${images.length} total):
${imageList}

TASK:
Look at the screenshot CAREFULLY. This is a visual spreadsheet with product rows.

For each product in the list above, determine which IMAGE (by number from the image list) is most likely displayed in that product's row based on the VISUAL LAYOUT of the spreadsheet.

The images are embedded in the spreadsheet cells. Look at the visual layout:
- Product names/text are in columns on the left
- Product images are in a column on the right
- Each row has one product and one image next to each other

IMPORTANT: The images in the spreadsheet are actual product photos (dining chairs). Match each product to the image that appears in its row based on the SPATIAL POSITION (which image thumbnail is horizontally aligned with which product text).

Return a JSON array mapping each product to its image:
[
  { "productIndex": 0, "imageIndex": 0, "confidence": 95, "reason": "Product 1's row has image 1 aligned next to it" },
  { "productIndex": 1, "imageIndex": 2, "confidence": 88, "reason": "Product 2's row has image 3 aligned next to it" }
]

RULES:
- productIndex: 0-based index from the product list above
- imageIndex: 0-based index from the image list above
- confidence: 0-100 how sure you are this mapping is correct based on visual row alignment
- If you can't determine the mapping for a product, set confidence to 0
- If multiple products share the same image (repeated), that's OK
