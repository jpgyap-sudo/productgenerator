...
The spreadsheet has product rows. Each row contains product information AND an embedded image of that product.
...
TASK:
Look at the screenshot CAREFULLY. For each product row, identify which image (by number from the list above) is displayed in that row.

The images are embedded in the spreadsheet cells. Look at the visual layout:
- Product names/text are in one column
- Product images are in another column (likely the rightmost column)
- Each row has one product and one image next to each other

Return a JSON array mapping each product to its image:
[
  { "productIndex": 0, "imageIndex": 0, "confidence": 95, "reason": "Product 1's image shows a dining chair matching the description" },
  { "productIndex": 1, "imageIndex": 2, "confidence": 88, "reason": "Product 2's image appears to show a different chair style" }
]
