...
const MAX_IMAGES_TO_RETURN = 20; // Max images to return in response (for legacy)
const MAX_IMAGES_FOR_MATCH = 200; // Max images to return for matching (Phase 2)
...
export async function extractImagesFromZip(zipBuffer) {
  ...
  // Score each image (keep quality metadata) but preserve original ZIP entry order
  const scored = imageEntries.map(img => ({
    ...img,
    score: calculateImageScore(img)
  }));

  // Select the best image by score (without reordering the array)
  let selectedImage = scored[0] || null;
  for (const img of scored) {
    if (!selectedImage || img.score > selectedImage.score) {
      selectedImage = img;
    }
  }

  // Prepare response — preserve original ZIP/filename order so that
  // imageIndex references are stable and the gallery displays sequentially.
  // Only include base64 data for the selected image to keep JSON small.
...
/**
 * Extract ALL images from a ZIP buffer with full data URLs.
 * Used by Phase 1 (Extract & Inspect) and Phase 2 (Match & Preview).
 * Returns up to MAX_IMAGES_FOR_MATCH images with base64 data for preview.
 *
 * @param {Buffer} zipBuffer - The ZIP file buffer
 * @returns {Promise<{images: Array, totalImages: number}>}
 */
export async function extractAllImagesFromZip(zipBuffer) {
  if (!zipBuffer || !Buffer.isBuffer(zipBuffer)) {
    throw new Error('ZIP buffer is required');
  }
  ...
  console.log(`[ZIP-EXTRACTOR] Extracting ALL images from ZIP (${(zipBuffer.length / 1024 / 1024).toFixed(2)} MB)`);
  ...
  for (const entry of entries) {
    ...
    if (data.length > MAX_IMAGE_SIZE_BYTES) continue;
  ...
  // Limit to MAX_IMAGES_FOR_MATCH to avoid browser memory issues
  const limited = imageEntries.slice(0, MAX_IMAGES_FOR_MATCH);
