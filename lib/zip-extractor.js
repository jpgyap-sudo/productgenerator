...
export async function extractImagesFromZip(zipBuffer) {
  ...
  // Prepare response (without raw data for non-selected images)
  const images = scored.slice(0, MAX_IMAGES_TO_RETURN).map(img => ({
    ...
    // Only include base64 data for the selected image and top 5 candidates
    dataUrl: (img === selectedImage || scored.indexOf(img) < 5)
