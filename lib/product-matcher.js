...
export function matchProductsToImages(products, images, options = {}) {
  ...
  // ── Sequential fallback: assign remaining unmatched products to remaining images ──
  // This is a last resort for catalogs where product codes don't match image filenames
  // at all (e.g. CH-790 in PDF vs xref14 in ZIP). The heuristic assumes the PDF lists
  // products in the same order as images appear in the ZIP.
  if (useSequentialFallback) {
    const unmatchedProducts = matches.filter(m => !m.matchedImage);
    const availableImages = [];
    for (let ii = 0; ii < images.length; ii++) {
      if (!usedImageIndices.has(ii)) {
        availableImages.push({ imageIndex: ii, ...images[ii] });
      }
    }

    if (unmatchedProducts.length > 0 && availableImages.length > 0) {
      const assignCount = Math.min(unmatchedProducts.length, availableImages.length);
      for (let i = 0; i < assignCount; i++) {
        const um = unmatchedProducts[i];
        const img = availableImages[i];
        um.matchedImage = { ...img, imageIndex: img.imageIndex, score: 10, matchType: 'sequential-fallback' };
        um.score = 10;
        um.matchType = 'sequential-fallback';
        usedImageIndices.add(img.imageIndex);
      }
    }
  }
