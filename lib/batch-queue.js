...
import { saveBatchImageMetadata } from './upload-gallery.js';
...
// ── Pause/Resume state ──────────────────────────────────────────
// In-memory map of paused batch IDs → resolve function
// When a batch is paused, the pipeline awaits a promise that resolves on resume.
const pausedResolvers = new Map();
...
export async function updateBatchJob(batchId, updates) {
  const allowedFields = [
    ...
    'paused_at', 'resume_state', 'all_images'
  ];
 ...
 * Saves the current state as `resume_state` so the pipeline can be resumed later.
 *
 * @param {number} batchId - Batch job ID
 * @returns {Promise<object>} Updated batch state
 */
export async function pauseBatch(batchId) {
  const state = await getBatchState(batchId);
  if (!state) throw new Error(`Batch ${batchId} not found`);

  // Can only pause running/processing batches
  const pausableStatuses = ['extracting_pdf', 'fingerprinting_zip', 'filtering_candidates', 'verifying_with_openai', 'retrying_failed'];
  if (!pausableStatuses.includes(state.status)) {
    throw new Error(`Cannot pause batch in status "${state.status}". Only processing batches can be paused.`);
  }

  // Save resume state — a snapshot of current progress
  ...
  await updateBatchJob(batchId, {
    ...
    current_item: 'Pipeline paused'
  });

  await addActivityLog(batchId, 'Pipeline paused by user');

  console.log(`[BATCH-QUEUE] Paused batch ${batchId}`);
  return getBatchState(batchId);
}

/**
 * Resume a paused batch job.
 * Restores the status/stage from `resume_state` and resolves the in-memory pause promise
...
export async function resumeBatch(batchId) {
  ...
  if (state.status !== 'paused') {
    throw new Error(`Cannot resume batch in status "${state.status}". Only paused batches can be resumed.`);
  }

  // Restore previous state from resume_state
  let resumeState = {};
  if (state.resume_state) {
    try {
      resumeState = typeof state.resume_state === 'string'
        ? JSON.parse(state.resume_state)
        : state.resume_state;
    } catch { /* ignore parse errors */ }
  }

  const restoredStatus = resumeState.status || 'queued';
  const restoredStage = resumeState.stage || 'Resumed';
  ...
  await updateBatchJob(batchId, {
    ...
    paused_at: null,
    resume_state: null,
    current_item: resumeState.current_item || 'Resuming...',
    estimated_seconds_remaining: resumeState.estimated_seconds_remaining || null
  });

  await addActivityLog(batchId, 'Pipeline resumed by user');

  // Resolve the in-memory pause promise so the pipeline continues
  const resolver = pausedResolvers.get(batchId);
  if (resolver) {
    resolver();
    pausedResolvers.delete(batchId);
  }

  console.log(`[BATCH-QUEUE] Resumed batch ${batchId}`);
  return getBatchState(batchId);
}

/**
 * Check if a batch is paused and wait until resumed.
 * Called periodically within the pipeline loop.
 * If the batch is paused, this function blocks until resumeBatch() is called.
 *
 * @param {number} batchId - Batch job ID
 * @returns {Promise<void>}
 */
async function checkPaused(batchId) {
  const state = await getBatchState(batchId);
  if (!state || state.status !== 'paused') return;

  console.log(`[BATCH-QUEUE] Batch ${batchId} is paused — waiting for resume...`);

  // Create a promise that resolves when resumeBatch() is called
  await new Promise((resolve) => {
    pausedResolvers.set(batchId, resolve);
  });

  console.log(`[BATCH-QUEUE] Batch ${batchId} resumed — continuing pipeline`);
}

/**
 * Auto-resume any paused batches on server startup.
 * Called once when the server starts.
 */
export async function autoResumePausedBatches() {
  try {
    const { data: pausedBatches, error } = await supabase
      .from(BATCH_JOBS_TABLE)
      .select('id, stage, resume_state')
      .eq('status', 'paused')
      .limit(10);

    if (error) {
      console.error(`[BATCH-QUEUE] Failed to fetch paused batches for auto-resume: ${error.message}`);
      return;
    }

    if (!pausedBatches || pausedBatches.length === 0) return;

    console.log(`[BATCH-QUEUE] Auto-resuming ${pausedBatches.length} paused batch(es)...`);

    for (const batch of pausedBatches) {
      let resumeState = {};
      if (batch.resume_state) {
        try {
          resumeState = typeof batch.resume_state === 'string'
            ? JSON.parse(batch.resume_state)
            : batch.resume_state;
        } catch { /* ignore */ }
      }

      const restoredStatus = resumeState.status || 'queued';
      const restoredStage = resumeState.stage || 'Resumed (auto)';
      ...
      await updateBatchJob(batch.id, {
        ...
        paused_at: null,
        resume_state: null,
        current_item: 'Auto-resumed after server restart',
        estimated_seconds_remaining: resumeState.estimated_seconds_remaining || null
      });

      await addActivityLog(batch.id, 'Pipeline auto-resumed after server restart');
      console.log(`[BATCH-QUEUE] Auto-resumed batch ${batch.id} → ${restoredStatus}`);
    }
  } catch (err) {
    console.error(`[BATCH-QUEUE] Auto-resume error: ${err.message}`);
  }
}
...
export async function runBatchPipeline(params, options = {}) {
  ...
  // ── Store image metadata (without dataUrl) to filesystem for later retrieval ──
  // Uses filesystem (batch-images.json) instead of DB because the Supabase
  // connection pooler is not enabled for this project, preventing DDL migrations.
  if (images && images.length > 0) {
    await saveBatchImageMetadata(batchId, images);
  }
