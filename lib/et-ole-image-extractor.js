...
function parseEmbeddedZip(buffer) {
  ...
  for (let i = 0; i < buffer.length - 30; i++) {
    ...
    if (buffer.readUInt32LE(i) === ZIP_LOCAL_HEADER_SIG) {
      ...
      // Validate: fileNameLen must be reasonable (max 260 chars for ZIP filenames)
      if (fileNameLen < 1 || fileNameLen > 260) {
        // False positive PK\x03\x04 — skip this byte and continue scanning
        continue;
      }

      // Validate: compressedSize must be within buffer bounds
      const dataStart = i + 30 + fileNameLen + extraLen;
      if (dataStart + compressedSize > buffer.length) {
        // Entry data extends beyond buffer — false positive
        continue;
      }

      const fileName = buffer.slice(i + 30, i + 30 + fileNameLen).toString('utf8');

      // Validate: filename must be reasonable ASCII (no null bytes, no control chars except forward slash)
      let validName = true;
      for (let ci = 0; ci < fileName.length; ci++) {
        const code = fileName.charCodeAt(ci);
        if (code === 0 || (code < 32 && code !== 13 && code !== 10)) {
          validName = false;
          break;
        }
      }
  ...
  // ── Post-processing: validate and deduplicate entries ────────────
  // Remove entries with zero compressed size (likely false positives)
  const validEntries = entries.filter(e => e.compressedSize > 50);

  // Deduplicate by fileName (keep first occurrence)
  const seenNames = new Set();
  const deduped = [];
  for (const e of validEntries) {
    if (!seenNames.has(e.fileName)) {
      seenNames.add(e.fileName);
      deduped.push(e);
    }
  }

  if (deduped.length !== entries.length) {
    console.log(`[ET-OLE] ZIP parsing: ${entries.length} raw entries → ${deduped.length} valid after validation/dedup`);
  }
