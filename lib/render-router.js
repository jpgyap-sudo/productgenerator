 ...
 */
export async function renderSingleView({ originalImagePath, originalImageUrl, productName, brand, mode, view }) {
  ...
  try {
    ...
    if (mode === 'gemini-only') {
      ...
      const geminiResult = await generateGeminiView(viewObj, productName || '', originalImageUrl, '1K', brand || '');
