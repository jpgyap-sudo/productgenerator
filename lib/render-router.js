  ...
  try {
    ...
    if (qa.decision === 'fix') {
      ...
      for (let attempt = 1; attempt <= MAX_FIX_ATTEMPTS; attempt++) {
        ...
        const currentUrl = await uploadToSupabase(currentPath, currentKey);

        // Gemini fixer: receives original + generated, outputs fixed version
        const geminiResult = await generateGeminiView(
        ...
        if (qa.decision === 'pass' || attempt === MAX_FIX_ATTEMPTS) {
          ...
          return {
            view,
            status: qa.decision === 'pass' ? 'fixed' : 'fixed',
            imageUrl: publicUrl,
            qaScore: qa.score,
            qaDecision: qa.decision,
            qaNotes: qa.notes,
