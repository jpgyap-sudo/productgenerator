...
export async function withRetry(fn, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
    try {
      ...
      const willRetry = attempt <= maxRetries && shouldRetry(err, attempt);
      ...
      if (willRetry) {
        ...
        // Log non-retryable errors so we can diagnose what's happening
        console.log(`[RETRY-MANAGER] Attempt ${attempt}/${maxRetries + 1} failed, NOT retrying: ${err.message}`);
        break;
      }
    }
  }
