...
// Note: The v1/generation endpoint is deprecated.
// The v2beta/stable-image/generate/sdxl endpoint uses a different request format.
// We keep v1 for now since it works with the current image-to-image flow,
// but plan to migrate to v2beta in a future update.
...
function targetDimensionsForView(viewId) {
  return Number(viewId) === 4
    ? { width: 1216, height: 832 }
    : { width: 832, height: 1216 };
}

function imageStrengthForView(viewId) {
...
function getEncodedImageDimensions(buffer) {
...
function assertStabilityDimensions(buffer, target, label) {
  const actual = getEncodedImageDimensions(buffer);
  if (!actual || actual.width !== target.width || actual.height !== target.height) {
    const actualText = actual ? `${actual.width}x${actual.height}` : 'unknown';
    throw new Error(
      `Stability init image normalization failed for ${label}: expected ${target.width}x${target.height}, got ${actualText}`
    );
  }
}
...
async function normalizeInitImage(buffer, viewId) {
  ...
  assertStabilityDimensions(outputBuffer, target, `view ${viewId}`);
...
export async function generateStabilityView(view, desc, imageUrl, resolution = '1K', brand = '', options = {}) {
  ...
  // Reference image
  const imageFile = new File([new Uint8Array(initImage.buffer)], 'reference.png', { type: initImage.mimeType });
  ...
  formData.append('image_strength', imageStrengthForView(view.id));
